...
for((i=1;i<=$folds;i++));do
    ...
    # rr mean
    sh scriptes/distribued.sh $lambda_krr $gamma $servers $workers ${file_name} $feature_size rr mean >> $log_file_rr_mean

    # rr mdd
    sh scriptes/distribued.sh $lambda_krr $gamma $servers $workers ${file_name} $feature_size rr mdd >> $log_file_rr_mdd

    # krr mean
    sh scriptes/distribued.sh $lambda_krr $gamma $servers $workers ${file_name} $feature_size krr mean >> $log_file_krr_mean

    # krr mdd
    sh scriptes/distribued.sh $lambda_krr $gamma $servers $workers ${file_name} $feature_size krr mdd >> $log_file_krr_mdd
